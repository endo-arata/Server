...
要求 ─► [Router]
          ...
          ▼
   Titan / Heavy / Fast / Sense / Cloud(承認制: D-015)
```
...
### クラウド API の扱い(D-015: 承認制)

1. Router はまずローカル Tier で回答を試みる
2. 「自信度が低い」「ローカルで失敗(ツール呼び出し不整合・タイムアウト)」「オーナーが明示要求」のいずれかで**クラウド候補**を生成
3. 送信予定の内容(プロンプト全文・添付・推定コスト)を Discord に提示し、ボタンで承認
4. 承認時のみ送信。機微カテゴリ(認証情報・健康・家計・住所等)を含む場合は自動で送信不可にし、マスキング案を提示
5. 結果と費用は監査ログへ。月次でクラウド依存率を Grafana に表示(目標: 5% 未満)

対象候補: Claude(Anthropic)を第一。API キーは秘密管理サービス経由でツール側が保持し、エージェントには渡さない。

## 4.4 評価ハーネス(モデル入替の判断根拠)

- 個人用ベンチセット(**日本語会話は必須項目**・要約・英日翻訳要約・コーディング・推論・ツール呼び出し・長文)各 20〜50 問(D-019)
